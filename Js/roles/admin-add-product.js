...
window.addEventListener('load', function() {
    ...
    //get sellers and display at sellect
    getUserByRole("seller").then((data) => {
        productSellerInput.innerHTML=''
        data.forEach(s =>{
            productSellerInput.innerHTML += ` <option value="${s.id}" selected>${s.userName}</option>`
        })
    }).catch((err) => {
        console.error(err);
    });
